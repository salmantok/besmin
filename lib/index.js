...
async function processFile(
    ...
    try {
        ...
        console.error(`❌ ${moduleType}: ${inputPath}`);
        console.error(err);
    }
}
...
async function main() {
    ...
    const moduleTypes = args.filter((arg) =>
        ['cjs', 'sysjs', 'amd', 'umd', 'mjs'].includes(arg)
    );
    const isModuleTypes =
        '❌ Usage: <inputDir> <outputDir> [cjs|umd|amd|sysjs] (or default [mjs]) [--watch] [--fix]';
    ...
    const resolvedModuleTypes = moduleTypes.length > 0 ? moduleTypes : ['mjs'];
